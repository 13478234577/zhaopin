...
function sheng() {
    var o=document.getElementById("sheng");
    var i=document.getElementById("shi");
    i.innerHTML="<option value='请选择市'>请选择市</option>";
    if(o.value=="辽宁省")
    {
        for(var j=0;j<ln.length;j++)
        {
            i.innerHTML+="<option value='"+ln[j]+"'>"+ln[j]+"</option>";
        }
    }
    if(o.value=="吉林省")
    {
        ...
        {
            i.innerHTML+="<option value='"+jl[y]+"'>"+jl[y]+"</option>";
        }
    }
...
function init() {
    for(var i=0; i<provinceList.length; i++){　　　　//provinceList.length为省数组的长度，下标从0开始，所以定义var i=0
        ...
        　　　　　　　　　　//通过下标获取省列表（上面的列出列表）中的数据

        var provinceName = provinceList[i].name;　　　　　　　　//根据 province.name获取省的名字

        console.log(provinceName);　　　　　　　　　　　　　//这步可以不需要，为了方便在网页打印台上显示出来，方便我们获取数据



        // provinceTag.innerHTML="<option value='"+provinceName+"'>"+provinceName+"</option>";
